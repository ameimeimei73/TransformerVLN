...
learning_rate = 3e-5
weight_decay = 0.0001
n_iters = 50000 if feedback_method == 'teacher' else 20000
...
def train(train_env, model, n_iters, log_every=2000, val_envs={}, model_type=None):
    ...
    for idx in range(0, n_iters, log_every):
        ...
        agent.train(optimizer, interval, feedback=feedback_method, model_type=model_type)
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            agent.test(use_dropout=True, feedback=feedback_method, allow_cheat=True, model_type=model_type)
            val_losses = np.array(agent.losses)
            val_loss_avg = np.average(val_losses)
            data_log['%s loss' % env_name].append(val_loss_avg)
            # Get validation distance from goal under test evaluation conditions
            agent.test(use_dropout=False, feedback='argmax', model_type=model_type)
            agent.write_results()
...
def train_val():
    ...
    if our_model == 'BERT+FC':
        ...
        exit()
    train(train_env, model, n_iters, val_envs=val_envs, model_type=our_model)
